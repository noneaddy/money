...
* iOS IPA Analysis works only on Mac OSX and above.
...
***Tip:*** Lazy to setup Static Analysis?
Use the docker build from [DockerHub](https://hub.docker.com/r/opensecurity/mobile-security-framework-mobsf/). 
(iOS IPA analysis won't work if you are using the docker container. Dynamic analysis mostly need you to configure MobSF in host OS than inside a container.) 
...
If you need to run on a specific port number try  `python3 manage.py runserver PORT_NO`.
To expose MobSF to a particular IP, you can try `python3 manage.py runserver IP:PORT_NO`.
...
2. [Dynamic Analysis with MobSF Android 4.1.2 arm Emulator - (Slow, Most Apps work)](https://github.com/MobSF/Mobile-Security-Framework-MobSF/wiki/1.-Documentation/#configuring-dynamic-analyzer-with-with-mobsf-android-412-arm-emulator) (**BROKEN**)
