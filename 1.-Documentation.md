...
* Android Studio and a configured virtual device is required if your using MobSF ARM Emulator. Intel HAXM is recommended.
...
1. [Dynamic Analysis with MobSF Android 4.4.2 x86 VirtualBox VM - **default** (Fast, not all Apps work)](https://github.com/MobSF/Mobile-Security-Framework-MobSF/wiki/1.-Documentation/_edit#configuring-dynamic-analyzer-with-mobsf-android-442-x86-virtualbox-vm)
2. [Dynamic Analysis with MobSF Android 4.1.2 arm Emulator - (Slow, Most Apps work)](https://github.com/MobSF/Mobile-Security-Framework-MobSF/wiki/1.-Documentation/_edit#configuring-dynamic-analyzer-with-with-mobsf-android-412-arm-emulator)
3. [Dynamic Analysis using a Rooted Android 4.03 - 4.4 Device (Very Fast, All Apps work)](https://github.com/MobSF/Mobile-Security-Framework-MobSF/wiki/1.-Documentation/_edit#configuring-dynamic-analyzer-with-rooted-android-403---44-device)
4. [Dynamic Analysis using a Rooted Android 4.03 - 4.4 VirtualBox VM (not tested)](https://github.com/MobSF/Mobile-Security-Framework-MobSF/wiki/1.-Documentation/_edit#configuring-dynamic-analyzer-with-rooted-android-403---44-virtualbox-vm)
...
* Extract MobSF_ARM_Emulator.zip (Downloaded already)
...
* Run `mobsfy_AVD.py` script and provide the directory that contains the files extracted from `MobSF_ARM_Emulator.zip`. This will automatically populate `MobSF/settings.py
* In `MobSF/settings.py`, set `ANDROID_DYNAMIC_ANALYZER = MobSF_AVD`

#### Manual Configuration (not recommended)
* If `mobsf_AVD.py` is not running successfully, you need to set the values for `AVD_EMULATOR` and `AVD_PATH` in `MobSF/settings.py` manually.
...
### Configuring Dynamic Analyzer with Rooted Android 4.03 - 4.4 VirtualBox VM
