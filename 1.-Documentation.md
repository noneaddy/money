...
(Download if you need Dynamic Analysis)
...
* For Android Device:
  ...
  * Sample configuration will look like this.
    ![Dynamic Analysis on Android Device](https://cloud.githubusercontent.com/assets/4301109/13613668/1b704718-e593-11e5-9d74-44e83ea5acf5.png)


* For Custom Android VM
  
  *  **VM on Virtual Box**: If the VM is hosted on VirtualBox, follow the same steps that you have followed for configuring MobSF VM and set appropriate `VM  UUID`, `Snapshot UUID`, `Host/Proxy IP`, and `VM/Device IP`. Also make sure that `REAL_DEVICE` is set to `False`
